...
from llmcompress.algorithm.quantization import *
from llmcompress.algorithm.sparsification import *
from llmcompress.data import BaseDataset, BaseTokenizer
from llmcompress.eval import PerplexityEval, TokenConsistencyEval
from llmcompress.models import *
from llmcompress.utils import (check_config, mkdirs, print_important_package_version,
                        seed_all, update_autoawq_quant_config,
                        update_vllm_quant_config)
from llmcompress.utils.registry_factory import ALGO_REGISTRY, MODEL_REGISTRY
...
def main(config):
    ...
    if 'save' in config and config.save.get('save_trtllm', False):
        ...
        from llmcompress.utils.export_trtllm import cvt_trtllm_engine
    ...
    if 'opencompass' in config:
        ...
        opencompass_cmd = (
            ...
            f'--llmcompress_cfg {args.config} '
            f'--llmcompress_eval_mode quant '
            f'--llmcompress_model_path {eval_model_path}'
        )
...
if __name__ == '__main__':
    llmcompress_start_time = time.time()
    ...
    llmcompress_end_time = time.time()
    llmcompress_duration_time = llmcompress_end_time - llmcompress_start_time
    logger.info(f'llmcompress_duration_time: {llmcompress_duration_time} s')
    logger.info('--- llmcompress finished ---')
